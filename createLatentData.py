...
training_data, validation_data, training_loader, validation_loader, x_train_var = utils.load_data_and_data_loaders('TANG', 10)

allLatents = []
for img in training_loader:

    encoding = model.encoder(img.to(device))
    _, z_q, _, _, _  = model.vector_quantization(model.pre_quantization_conv(encoding))
    allLatents.append(z_q.detach().cpu().numpy())
allLatents = numpy.concatenate(allLatents)
np.save("LatentM2S1", allLatents)
...
for img in validation_loader:
    ...
    encoding = model.encoder(img.to(device))
    _, z_q, _, _, _  = model.vector_quantization(model.pre_quantization_conv(encoding))
    allLatentsVal.append(z_q.detach().cpu().numpy())


allLatentsVal = numpy.concatenate(allLatentsVal)

np.save("LatentM2S1Val",allLatentsVal)
